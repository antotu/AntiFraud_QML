...
message = f"Strongly,{numElements},{numFeatures},{encoding},{numLayers},{reup} starts"    
...
with torch.inference_mode():
    ...
    #message = f"Strongly,{numElements},{numFeatures},{encoding},{numLayers},{reup} | Test loss: {test_loss:4f} | Test acc: {test_acc:4f}"    
    #url = f"https://api.telegram.org/bot{TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
    #requests.get(url).json()
...
for epoch in tqdm(range(epochs)):
    ...
    with torch.inference_mode():
        ...
        #message = f"Epoch {epoch} | Strongly,{numElements},{numFeatures},{encoding},{numLayers},{reup} | Train loss: {train_loss:4f} | Train acc: {train_acc:4f} | Test loss: {test_loss:4f} | Test acc: {test_acc:4f}"    
        #url = f"https://api.telegram.org/bot{TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
        #requests.get(url).json()
...
fileTest = open(f"confTestStrongly_{numElements}_{numFeatures}_{encoding}_{numLayers}_{reup}.csv", "w")
#fileTest.write(name_conf + ",")
np.savetxt(fileTest, confMatrixTest, fmt="%d", delimiter=",", newline=",")
fileTest.write("\n")
"""

message = f"Strongly,{numElements},{numFeatures},{encoding},{numLayers},{reup} ends"    
url = f"https://api.telegram.org/bot{TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
requests.get(url).json()
