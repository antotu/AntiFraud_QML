...
class VQCBasicEntanglingLayerModel(nn.Module):
    """
    VQC 
    PARAMS:
    encoding: str, type of encoding chosen
    numLayer: int, number of layer for the basic entangling layer ansatz
    ...
    def __init__(self, encoding: str,
        ...
        super().__init__()
        # set the parameters related to the Basic Entangling layer part 
        shape = qml.BasicEntanglerLayers.shape(n_layers=numLayer, n_wires=numWires)
        w1 = np.pi * np.random.random(shape, requires_grad=True)
        self.weights = nn.Parameter(torch.from_numpy(w1), requires_grad=True)
        # set the encoding 
        self.encoding = encoding
        # set the gates applied
        self.gates = gates
        # set the number of layer for the Basic entangling circuit part
    ...
    def circuit(self, features):
        ...
        if self.reUploading:
            ...
            for i in self.weights:
                ...
                qml.BasicEntanglerLayers(i.unsqueeze(0), wires=range(self.num_wires))
        else:
            # otherwise apply just encoding and ansatz
            self.set_encoding(features=features)
            qml.BasicEntanglerLayers(self.weights, wires=range(self.num_wires))
